...
class Dashboard extends Component {
  ...
  componentWillMount() {
    axios.get(`http://localhost:3300`)
      .then(res => {
        this.setState({ resReceived: res.data })
        // console.log("resReceived",this.state.resReceived)
      })
  }
  render() {
    ...
    return (
      <div className="content">
        {console.log("render->", this.state.resReceived)}
        <Grid fluid>
          <Row>
            <Col lg={3} sm={6}>
              <StatsCard
                    ...
                    <div>
                      <Table style={{ width: "500px" }}>
                        <tbody className="tableBody">
                          {console.log(this.state.resReceived.tdArrayMap)}
                         
                          {
                            this.state.resReceived.tdArrayMap && this.state.resReceived.tdArrayMap.length > 0 && (this.state.resReceived.tdArrayMap).map((prop, key) => {
                              return (
            ...
            <Col md={4}>
              <Card
                ...
                content={
                  ...
                  >
                    <ChartistGraph data={this.state.resReceived.dataPie && this.state.resReceived.dataPie.length > 0 && this.state.resReceived.dataPie} type="Pie" />
                  </div>
                }
                legend={
                  // console.log("acnoiean",this.state.resReceived.legendPie && this.state.resReceived.legendPie.length > 0 && this.createLegend(this.state.resReceived.legendPie))
                  <div className="legend">{this.state.resReceived.legendPie && this.createLegend(this.state.resReceived.legendPie)}</div>
                }
              />
            </Col>
            <Col md={8}>
              <Card
                ...
                content={
                  <div className="ct-chart">
                    <ChartistGraph
                      data={this.state.resReceived.dataSales && this.state.resReceived.dataSales.length > 0 &&this.state.resReceived.dataSales}
                      type="Line"
                      options={this.state.resReceived.optionsSales && this.state.resReceived.optionsSales.length > 0 &&this.state.resReceived.optionsSales}
                      responsiveOptions={this.state.resReceived.responsiveSales && this.state.resReceived.responsiveSales.length > 0 &&this.state.resReceived.optionsSales}
                    />
                  </div>
                }
                legend={
                  <div className="legend">{this.state.resReceived.legendSales && this.state.resReceived.legendSales.length > 0 }</div>
                }
              />
            </Col>
          </Row>
            ...
            <Col md={6}>
              <Card
                ...
                content={
                  <div className="ct-chart">
                    <ChartistGraph
                      data={this.state.resReceived.dataBar}
                      type="Bar"
                      options={this.state.resReceived.optionsBar}
                      responsiveOptions={this.state.resReceived.responsiveBar}
                    />
                  </div>
                }
                legend={
                  <div className="legend">{this.state.resReceived.legendBar && this.state.resReceived.legendBar.length > 0 && this.createLegend(this.state.resReceived.legendBar)}</div>
                }
              />
            </Col>
